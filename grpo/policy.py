...
class BasePolicy:
    ...
    def act(self, observations, return_log_prob = False):
        ...
        sampled_actions, log_probs = [], []
        for observation in observations:
            distn = self.action_distribution(observation)
            action = distn.sample()
            log_prob = distn.log_prob(action)
            sampled_actions.append(action.detach().numpy())
...
class GaussianPolicy(BasePolicy, nn.Module):
    ...
    def action_distribution(self, observations):
        ...
        cov = torch.diag(self.std())
        distribution = ptd.MultivariateNormal(loc=mean, covariance_matrix=cov)
        #######################################################
        #########          END YOUR CODE.          ############
        return distribution
